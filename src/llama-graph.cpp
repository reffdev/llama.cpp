...
void llm_graph_input_mtp::set_input(const llama_ubatch * ubatch) {
    if (!ubatch || ubatch->n_tokens != 1) {
        // During warmup or graph estimation, ubatch may not match MTP expectations. Skip.
        return;
    }
    if (!ubatch->token || !ubatch->embd) {
        return;
    }
...
void llm_graph_input_attn_kv_iswa::set_input(const llama_ubatch * ubatch) {
    // MTP graphs only read from the KV cache — K/V write indices may not be part
    // of the scheduled subgraph and therefore unallocated (buffer == NULL). Also,
    // base tensors may be absent if there are no non-SWA attention layers. Set each
    // tensor only if it was actually allocated.
