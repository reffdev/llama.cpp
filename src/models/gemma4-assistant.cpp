...
static void gemma4_mtp_build_one_step(
    ...
    auto * ctx0    = gctx.ctx0;
    auto * gf      = gctx.gf;
    const auto & hparams = mtp.hparams;  // MTP model's own hparams for layer iteration
    const auto & cparams = gctx.cparams;
    const int    n_layer = (int) mtp.hparams.n_layer;  // 4 MTP layers, not 42 target layers
    ...
    // Gemma 4 scales token embeddings by sqrt(n_embd) at the input pipeline.
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        cur = gctx.build_attn_mtp(inp_attn, mtp.layers[il].wo, nullptr, Qcur, nullptr, nullptr, nullptr,
                hparams.f_attention_scale, il, il_kv, read_swa);
    ...
    if (mtp.hparams.use_ordered_embeddings) {
        // Centroid-routed LM head (HF Gemma4AssistantMaskedEmbedder)
...
void llama_model_gemma4_assistant::load_arch_hparams(llama_model_loader & ml) {
    ...
    // NOTE: the assistant is never given its own context/MTP graph — it is nested in the
    // gemma4 target and its MTP graph is built via the target (llama_model_gemma4::build_arch_graph).
    // Do NOT set nextn_predict_layers here: upstream's load-time graph reserve would otherwise build
    // a DECODER_MTP graph for the standalone assistant (with no real target), tripping a mul_mat assert.
...
std::unique_ptr<llm_graph_context> llama_model_gemma4_assistant::build_arch_graph(const llm_graph_params & params) const {
    if (params.gtype == LLM_GRAPH_TYPE_DECODER_MTP) {
        return std::make_unique<graph_mtp>(*this, *this, params);
    }
    throw std::runtime_error(
        "gemma4_assistant cannot be used as a primary model (-m). "
        "Use --spec-type draft-mtp with -md to load it as a speculative draft model.");
}

//
// graph_mtp: MTP speculative decoding graph
//
...
llama_model_gemma4_assistant::graph_mtp::graph_mtp(
    ...
    // Single-step MTP build: one draft step per graph compute.
