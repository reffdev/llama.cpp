...
llama_kv_cache::slot_info llama_kv_cache::mtp_slot_info(llama_seq_id seq_id) const {
    ...
    sinfo.s0 = st;
    sinfo.s1 = st;
    // strm[] indexes the per-stream v_cells/v_heads arrays (size n_stream), so it must
    // hold the STREAM index, not the seq_id. These differ under a unified KV cache
    // (n_stream == 1, all seqs -> stream 0); using seq_id read v_cells out of bounds.
    sinfo.strm.push_back(st);
