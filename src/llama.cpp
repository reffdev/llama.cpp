...
int llama_model_load_mtp_from_file(struct llama_model * model, const char * path_mtp, struct llama_model_params params) {
    ...
    // Skip vocab check if assistant uses "none" tokenizer (it shares the target's tokenizer at runtime)
    if (aux->vocab.get_type() != LLAMA_VOCAB_TYPE_NONE && !llama_mtp_vocab_matches(*tgt, *aux)) {
        llama_model_free(aux);
        return -7;
    }

    // Rename all MTP assistant tensors with "mtp." prefix so they can be
    // uniquely targeted by -ot rules without colliding with the main model's tensors.
    ...
    // Enable MTP context creation on the target model
    if (tgt->hparams.nextn_predict_layers == 0) {
        tgt->hparams.nextn_predict_layers = aux->hparams.n_layer;
    }
    return 0;
}
